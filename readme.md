...
> Options: `&show=reviews,discussions_started,discussions_answered,prs_merged,prs_merged_percentage,lines_added,lines_removed,lines_changed,github_actions`

> [!NOTE]\
> `lines_added`, `lines_removed`, `lines_changed` and `github_actions` aggregate data across every repository you own and have contributed to, using one GitHub REST request per repository. They are noticeably slower and more rate-limit intensive than the other stats, so they are only computed when explicitly requested via `&show=`. Use `lines_changed` to display the added/removed lines on a single line (e.g. `+300k -292k`), or `lines_added`/`lines_removed` for separate rows.
...
| `show` | Shows [additional items](#showing-additional-individual-stats) on stats card (i.e. `reviews`, `discussions_started`, `discussions_answered`, `prs_merged`, `prs_merged_percentage`, `lines_added`, `lines_removed`, `lines_changed` or `github_actions`). | string (comma-separated values) | `null` |
